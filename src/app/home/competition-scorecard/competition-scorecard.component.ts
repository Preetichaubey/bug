...
@Component({
  selector: 'app-competition-scorecard',
  templateUrl: './competition-scorecard.component.html',
  styleUrls: ['./competition-scorecard.component.scss'],
})
export class CompetitionScorecardComponent implements OnInit {
  ...
  ngOnInit() {
     this.dbService.GetPropertyAsPromise("CompetitionAgainstId").then(_aId => {
        if (_aId == '1') {
           this.AIWinner();
        } else if (_aId == '4') {
           this.GroupWinner();
  ...
  AIWinner() {
     ...
     this.dbService.GetPropertyAsPromise("CompetitionOppResult").then(_OppScore => {
        this.OppScore = +_OppScore;
        this.dbService.GetPropertyAsPromise("CompetitionMyResult").then(_myScore => {
           ...
           setTimeout(() => {
              if (this.OppScore > this.MyScore) {
                 ...
                 this.dbService.GetPropertyAsPromise("CompetitionMyCorrectTime").then(_myTime => {
                    this.dbService.GetPropertyAsPromise("CompetitionOppCorrectTime").then(_OppTime => {
                       if (_OppTime == "max") {
                          this.myClass = 'result-win';
                          this.MyIcon = "thumbs-up";
                          this.OppIcon = "thumbs-down";
  ...
  loadCrrQuizSummary() {
     eval('MathJax.Hub.Queue(["Typeset", MathJax.Hub])');
     this.dbService.GetPropertyAsPromise('CompetitioncurrQuiz').then(
        data => {
           console.log(data);
